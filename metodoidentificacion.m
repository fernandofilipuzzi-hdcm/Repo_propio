...
function noisy = add_white_noise_to_func(clean_signal, noise_amplitude)
	%#ADD_WHITE_NOISE_TO_FUNC agrega ruido blanco a una se馻l
	%#
	%# SYNOPSIS add_white_noise_to_func(clean_signal, noise_amplitude)
	%# INPUT clean_signal: (simb髄ico) la se馻l de entrada
	%# INPUT noise_amplitude: (float) amplitud de la se馻l de ruido
	%# OUTPUT noisy (simb髄ico) se馻l con ruido agregado
	%#
...
function [data_ident, data_validation] = generate_ident_package(input_signal, output_signal, sample_time, ident_proportion, plot_package)
	...
	%# INPUT input_signal: (simb髄ico) la se馻l de entrada
	%# INPUT output_signal (simb髄ico) la se馻l de salida
	...
	if plot_package
        	...
        	legend('Identificaci贸n', 'Validaci贸n');
...
function Gzi_mc = discrete_ident_recursive_least_squares(data, Ts, plot_ident)
	%#DISCRETE_IDENT_RECURSIVE_LEAST_SQUARES m閠odo de los minimos cuadrados
	%#
...
function analyze_residuals(data, sys_id, sampling_frequency)
    %#ANALYZE_RESIDUALS an醠isis de residuos
	%#
    ...
    % Correlaci贸n del residuo con la salida
    subplot(2, 2, 2); 
...
function validate_identifications(data, Gzi, Gzi_mc)
    ...
    title('Validaci贸n de resultados');
